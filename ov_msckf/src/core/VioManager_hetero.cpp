...
void feed_measurement_stereo_helper(double timestamp, 
                                    cv::Mat* img0_ptr, size_t img0_size,
    ...
    void *wrapper1 = __hetero_task_begin(6, 
                                        ...
                                        img0_ptr, img0_size, 
                                        img1_ptr, img1_size,
                                        obj, obj_size, "feed_measurement_stereo_wrapper1");
    
    double temp = timestamp;
    size_t cam_id0_temp = cam_id0;
    size_t cam_id1_temp = cam_id1;

    // Start timing
    obj->rT1 =  boost::posix_time::microsec_clock::local_time();

    // Assert we have good ids
    //assert(cam_id0_temp!=cam_id1_temp);

    // Feed our stereo trackers, if we are not doing binocular
    if(obj->params.use_stereo) {
        obj->trackFEATS->feed_stereo(temp, *img0_ptr, *img1_ptr, cam_id0_temp, cam_id1_temp);
    } else {
#ifdef ILLIXR_INTEGRATION
    ...
    __hetero_task_end(wrapper1);

    void *wrapper2 = __hetero_task_begin(6, 
                                        timestamp, 
                                        img0_ptr, img0_size, 
                                        img1_ptr, img1_size,
                                        cam_id0, 
                                        ...
                                        img0_ptr, img0_size, 
                                        img1_ptr, img1_size,
                                        "feed_measurement_stereo_wrapper2");

    // If aruoc is avalible, the also pass to it
    // NOTE: binocular tracking for aruco doesn't make sense as we by default have the ids
    // NOTE: thus we just call the stereo tracking if we are doing binocular!
    if(obj->trackARUCO != nullptr) {
        obj->trackARUCO->feed_stereo(timestamp, *img0_ptr, *img1_ptr, cam_id0, cam_id1);
    }
    obj->rT2 =  boost::posix_time::microsec_clock::local_time();

    __hetero_task_end(wrapper2);

    // If we do not have VIO initialization, then try to initialize
    // TODO: Or if we are trying to reset the system, then do that here!
    void *wrapper3 = __hetero_task_begin(1,
                                        obj, obj_size, 1,
                                        obj, obj_size, "feed_measurement_stereo_wrapper3");
    if(!obj->is_initialized_vio) {
        obj->is_initialized_vio = obj->try_to_initialize();
